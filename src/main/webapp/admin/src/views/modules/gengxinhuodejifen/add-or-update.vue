<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="订单编号" prop="dingdanbianhao">
          <el-select  @change="dingdanbianhaoChange" v-model="ruleForm.dingdanbianhao" placeholder="请选择订单编号">
            <el-option
                v-for="(item,index) in dingdanbianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        this.ruleForm.huodejifen = 0
	this.ro.huodejifen = false;
      }
      // 获取用户信息
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/xiaofeijilu/dingdanbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.dingdanbianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    dingdanbianhaoChange () {
      this.$http({
        url: `follow/xiaofeijilu/dingdanbianhao?columnValue=`+ this.ruleForm.dingdanbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.huodejifen){
                this.ruleForm.huodejifen = data.data.huodejifen
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    onSubmit() {
      ...
      if(obj!=null) {
	      obj.huodejifen = parseFloat(obj.huodejifen) + parseFloat(this.ruleForm.huodejifen)
	      this.$http({
		  url: `${table}/update`,
		  method: "post",
		  data: obj
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gengxinhuodejifen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gengxinhuodejifenCrossAddOrUpdateFlag = false;
                  this.parent.search();
