  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        this.ruleForm.shiyongjifen = 0
	this.ro.shiyongjifen = false;
      }
      // 获取用户信息
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jifenduihuan/shangpinbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    shangpinbianhaoChange () {
      this.$http({
        url: `follow/jifenduihuan/shangpinbianhao?columnValue=`+ this.ruleForm.shangpinbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shiyongjifen){
                this.ruleForm.shiyongjifen = data.data.shiyongjifen
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    info(id) {
      this.$http({
        url: `gengxinshiyongjifen/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      if(obj!=null) {
	      obj.shiyongjifen = parseFloat(obj.shiyongjifen) + parseFloat(this.ruleForm.shiyongjifen)
	      this.$http({
		  url: `${table}/update`,
		  method: "post",
		  data: obj
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gengxinshiyongjifen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gengxinshiyongjifenCrossAddOrUpdateFlag = false;
                  this.parent.search();
